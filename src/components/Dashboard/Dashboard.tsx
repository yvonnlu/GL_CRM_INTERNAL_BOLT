...
const Dashboard: React.FC = () => {
    ...
    // Sales Pipeline Overview - Base data
    const pipelineSubStages = [
        { stage: 'New', count: 89, value: 445000, color: 'bg-yellow-500' },
        { stage: 'Proposal', count: 34, value: 340000, color: 'bg-orange-500' },
        { stage: 'Negotiation', count: 12, value: 240000, color: 'bg-purple-500' },
    ];

    const leadsValue = pipelineSubStages.reduce((sum, stage) => sum + stage.value, 0);
    const leadsCount = pipelineSubStages.reduce((sum, stage) => sum + stage.count, 0);
    const closedWonValue = 160000;
    const totalPipelineValue = leadsValue + closedWonValue;
    ...
    const pipelineStages = [
        {
            ...
            count: leadsCount,
            value: leadsValue,
            formattedValue: formatCurrency(leadsValue),
            color: 'bg-blue-500',
            subStages: pipelineSubStages.map(stage => ({
                ...stage,
                formattedValue: formatCurrency(stage.value),
            })),
        },
        {
            stage: 'Closed Won',
            ...
            formattedValue: formatCurrency(closedWonValue),
            color: 'bg-green-500',
        },
    ]; // Team Members
    const teamMembers = [
        { name: 'Sarah Johnson', role: 'Sales Manager', clients: 45, revenue: '$145K' },
        { name: 'Mike Chen', role: 'Account Executive', clients: 38, revenue: '$120K' },
        { name: 'Emma Rodriguez', role: 'Marketing Lead', clients: 32, revenue: '$98K' },
        { name: 'David Thompson', role: 'Sales Rep', clients: 28, revenue: '$76K' },
    ];

    /* Conversion Rate Data - Keeping for future reference
                                    ...
                                    <div className="ml-6 space-y-2 mt-2">
                                        {stage.subStages.map((subStage, subIndex) => (
                                            ...
                                            >
                                                <div className="flex items-center space-x-3">
                                                    <div
                                                        className={`w-2 h-2 rounded-full ${subStage.color}`}
                                                    ></div>
                                                ...
                                                <div className="flex items-center space-x-4 text-xs text-gray-600">
                                                    <span>{subStage.count} deals</span>
                                                    <span className="font-semibold">
                                                        {subStage.formattedValue}
                                                    </span>
                                                </div>
                                            </div>
                                        ))}
                    ...
                    <div className="mt-6 pt-4 border-t border-gray-200">
                        <div className="flex items-center justify-between text-sm">
                            ...
                            <span className="font-bold text-gray-900">
                                {formatCurrency(leadsValue + closedWonValue)}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
                    ...
                    <div className="h-64 bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg flex items-center justify-center">
                        <div className="text-center">
                            ...
                            <p className="text-sm text-gray-500">
                                Lead to customer conversion visualization
                            </p>
                        </div>
                    </div>
                </div>
