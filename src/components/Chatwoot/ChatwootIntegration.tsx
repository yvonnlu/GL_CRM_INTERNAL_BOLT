...
import { MessageSquare, Phone, Mail, User, Clock, Send, Paperclip, MoreVertical, Quote, Star, Tag, UserPlus, FileText, CheckCircle, AlertCircle } from 'lucide-react';
...
const ChatwootIntegration: React.FC = () => {
    ...
    const mockConversations: ChatwootConversation[] = [
      {
        ...
        messages: [
          {
            id: 'msg-1',
            content: 'Hi, I\'m interested in your digital marketing services. Can you provide a quote for SEO and social media management for our tech company?',
          ...
          {
            id: 'msg-2',
            content: 'Hello Jennifer! Thank you for your interest. I\'d be happy to help you with a customized quote. Let me gather some information about your requirements.',
          ...
          {
            id: 'msg-3',
            content: 'We\'re a tech company with about 50 employees. We need to improve our online presence and generate more leads. Our budget is around $50,000 for the first 6 months.',
            messageType: 'incoming',
      ...
      },
      {
        ...
        messages: [
          {
            id: 'msg-4',
            content: 'Hi! Can we schedule a call to discuss the campaign performance? Also, I\'d like to get a quote for expanding our social media presence.',
            messageType: 'incoming',
      ...
      },
      {
        ...
        messages: [
          {
            id: 'msg-5',
            content: 'We need a comprehensive digital marketing strategy for our enterprise. Can you provide a detailed quote for SEO, PPC, content marketing, and social media management?',
            messageType: 'incoming',
  ...
  const sendMessage = () => {
    ...
    setConversations(prev => prev.map(conv => 
      conv.id === selectedConversation.id 
        ? { ...conv, messages: [...conv.messages, message], lastActivityAt: new Date().toISOString() }
        : conv
    ));
  ...
  const updateConversationStatus = (conversationId: string, status: 'open' | 'resolved' | 'pending') => {
    setConversations(prev => prev.map(conv => 
  ...
  const updateDealStage = (conversationId: string, stage: 'new' | 'qualified' | 'proposal' | 'negotiation' | 'closed-won' | 'closed-lost') => {
    setConversations(prev => prev.map(conv => 
      conv.id === conversationId ? { ...conv, dealStage: stage } : conv
    ));
  ...
  const markAsQuoteRequest = (conversationId: string) => {
    setConversations(prev => prev.map(conv => 
      conv.id === conversationId 
        ? { 
            ...conv, 
            isQuoteRequest: true, 
            labels: [...conv.labels.filter(l => l !== 'quote-request'), 'quote-request'],
            dealStage: 'qualified'
          }
        : conv
    ));
    ...
    if (selectedConversation?.id === conversationId) {
      setSelectedConversation(prev => prev ? { 
        ...prev, 
        isQuoteRequest: true,
        labels: [...prev.labels.filter(l => l !== 'quote-request'), 'quote-request'],
        dealStage: 'qualified'
      } : null);
    }
  };
  ...
  const assignConversation = (conversationId: string, assigneeId: string) => {
    setConversations(prev => prev.map(conv => 
      conv.id === conversationId ? { ...conv, assigneeId } : conv
    ));
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100vh-300px)]">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-4 border-b border-gray-200">
            <h3 className="font-semibold text-gray-900">Conversations ({filteredConversations.length})</h3>
          </div>
            ...
            {filteredConversations.map((conversation) => (
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center space-x-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{conversation.contact.company}</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-1">
                    <span className="text-lg">{getChannelIcon(conversation.inbox.channelType)}</span>
                ...
                {conversation.dealStage && (
                  <div className="mb-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${getDealStageColor(conversation.dealStage)}`}>
                ...
                {conversation.leadScore && (
                  <div className="mb-2">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-xs font-medium text-gray-900">{conversation.leadScore}</span>
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{selectedConversation.contact.company}</p>
                  ...
                  <div className="flex items-center space-x-2">
                    {!selectedConversation.isQuoteRequest && (
                      <button
                        onClick={() => markAsQuoteRequest(selectedConversation.id)}
                    ...
                    <select
                      ...
                      onChange={(e) => updateDealStage(selectedConversation.id, e.target.value as any)}
                      className="border border-gray-300 rounded px-2 py-1 text-sm"
                    >
                ...
                <div className="mt-3 p-3 bg-gray-50 rounded-lg">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <span className="ml-2 font-medium">{selectedConversation.contact.customAttributes.leadSource}</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Industry:</span>
                      <span className="ml-2 font-medium">{selectedConversation.contact.customAttributes.industry}</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Budget:</span>
                      <span className="ml-2 font-medium">{selectedConversation.contact.customAttributes.budget}</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Timeline:</span>
                      <span className="ml-2 font-medium">{selectedConversation.contact.customAttributes.timeline}</span>
                    </div>
                  </div>
                </div>
              </div>
